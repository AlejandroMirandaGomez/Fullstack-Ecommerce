...
function NavBar() {
  ...
  const currentRoute = category ? `${pathname}?category=${category}` : pathname;
  ...
  return (
    <nav className="xl:px-40 lg:px-15 md:px-5 flex">
      <div className="px-4 pb-4 w-full flex justify-between items-center flex-wrap md:gap-4 md:flex-nowrap ">
        <div className="flex justify-center items-center w-full  order-1 mt-4 md:mt-0 md:order-0 md:w-auto md:justify-end relative">
          <div
            onClick={() => setIsOpenSidebar((prev) => !prev)}
            className="flex justify-center items-center gap-2 cursor-pointer px-5 py-2 w-full text-white font-semibold bg-sky-400 rounded-2xl shadow-md"
          >
            <HamburgerMenu className="size-6" />
            <span className="md:hidden lg:inline">ALL CATEGORIES</span>
            <DownArrow
              className={
                "size-6 ml-2 md:hidden lg:inline transition-transform " +
